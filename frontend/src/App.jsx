...
export default function App() {
  ...
  useEffect(() => {
    ...
    const offActiveGame = on('active_game_found', ({ snapshot }) => {
      if (!snapshot) return;
    ...
    const offLeftActiveGame = on('left_active_game', () => {
      resetGameState();
      setScreen(SCREEN.LOBBY);
    });
    const offLeaveActiveErr = on('leave_active_error', ({ error }) => {
      setRollError(error);
    ...
    const offDiceRolled = on('dice_rolled', ({ userId, firstName, diceResult, isEliminated, remainingPlayers, players: updatedPlayers }) => {
      setRolling(false);
    ...
    const offReconnectTick = on('reconnect_tick', ({ userId, secondsLeft }) => {
      ...
      setMyReconnectSeconds(prev => prev !== null ? secondsLeft : prev);
    });
    ...
    const offSnapshot = on('game_snapshot', ({ game, players, activePlayers, currentPlayer, started, turnSecondsLeft, reconnectSecondsLeft }) => {
      ...
      if (started && game.status === 'active') {
        setScreen(SCREEN.GAME);
        if (reconnectSecondsLeft && reconnectSecondsLeft > 0) setMyReconnectSeconds(reconnectSecondsLeft);
    ...
    return () => {
      ...
      offTimerPaused(); offTimerResumed(); offJoined(); offJoinErr();
      offPlayerJoined(); offPlayerLeft(); offLeftGame(); offLeftActiveGame(); offLeaveActiveErr();
      offCountdownStart(); offCountdownTick(); offCountdownStopped();
      offReadyUpdated(); offAllReady(); offGameStarted(); offTurnTick(); offDiceRolled(); offRollErr();
  ...
  const handleReturnToGame = useCallback(() => {
    if (game?.room_code) emit('reconnect_game', { roomCode: game.room_code });
    setMyReconnectSeconds(null);
  }, [emit, game]);
  ...
  const ReferralToast = referralBonus ? (
    <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', background: 'linear-gradient(135deg, rgba(16,185,129,0.95), rgba(5,150,105,0.95))', border: '1px solid var(--success2)', borderRadius: 'var(--radius)', padding: '12px 20px', zIndex: 300, whiteSpace: 'nowrap', boxShadow: '0 8px 24px rgba(0,0,0,0.4)', animation: 'pop 0.3s ease' }}>
      <span style={{ fontWeight: 700, fontSize: '14px' }}>🎁 +{referralBonus.bonus} קרדיטים! {referralBonus.newUser} הצטרף דרך הקישור שלך</span>
    </div>
  ) : null;
