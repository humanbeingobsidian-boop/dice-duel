...
export default function WaitingRoomScreen({
  ...
  function handleReadyClick() {
    ...
    haptic('medium');
    onToggleReady();
    setTimeout(() => setReadyLocked(false), 800);
  }
    ...
    <div className="screen" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', paddingTop: '4px' }}>
        ...
        <button
          ...
          style={{ alignSelf: 'flex-start', padding: '8px 16px', fontSize: '13px', opacity: leaving ? 0.7 : 1 }}
      ...
      <div className="card" style={{ textAlign: 'center', padding: '20px' }}>
        {countdownActive ? (
          <>
            <div style={{ fontSize: '13px', color: 'var(--text3)', marginBottom: '8px' }}>{t('waiting_starts_in', lang)}</div>
            <div className="font-display" style={{ fontSize: '52px', fontWeight: 900, lineHeight: 1, color: timeLeft <= 10 ? 'var(--danger2)' : 'var(--gold2)', transition: 'color 0.3s' }}>{timeLeft}</div>
            <div style={{ fontSize: '13px', color: 'var(--text3)', marginTop: '4px' }}>{t('waiting_seconds', lang)}</div>
            <div style={{ height: '6px', background: 'var(--bg3)', borderRadius: '3px', marginTop: '14px', overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${progressPct}%`, background: timeLeft <= 10 ? 'linear-gradient(90deg, var(--danger), var(--danger2))' : 'linear-gradient(90deg, var(--accent), var(--gold2))', borderRadius: '3px', transition: 'width 1s linear, background 0.3s' }} />
            </div>
          </>
        ) : (
          <>
            ...
            <div style={{ color: 'var(--text3)', fontSize: '13px', marginTop: '6px' }}>{t('waiting_timer_hint', lang)}</div>
          </>
        )}
      </div>
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            {activePlayers.length >= 2 && <span style={{ fontSize: '13px', color: 'var(--text3)' }}>{readyCount}/{activePlayers.length} ready</span>}
            <span style={{ background: 'var(--accent)', color: 'white', padding: '3px 10px', borderRadius: '20px', fontSize: '13px', fontWeight: 600 }}>{activePlayers.length}/{maxPlayers}</span>
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
          {activePlayers.map(player => <PlayerSlot key={player.user_id} player={player} isMe={player.user_id === myUserId} isReady={readyPlayers.includes(player.user_id)} lang={lang} />)}
          {Array.from({ length: emptySlots }).map((_, i) => <EmptySlot key={`empty-${i}`} lang={lang} />)}
        </div>
      </div>
      ...
      {activePlayers.length >= 2 && (
        <button
          ...
          style={{ fontSize: '16px', padding: '14px', border: amIReady ? '2px solid var(--success2)' : undefined, color: amIReady ? 'var(--success2)' : undefined, opacity: readyLocked ? 0.65 : 1 }}
      ...
      <div style={{ background: 'linear-gradient(135deg, rgba(245,158,11,0.1), rgba(251,191,36,0.05))', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 'var(--radius)', padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ color: 'var(--text2)', fontSize: '14px' }}>{t('waiting_pot', lang)}</span>
        <span className="font-display" style={{ fontSize: '22px', color: 'var(--gold2)', fontWeight: 700 }}>{game?.pot ?? 0}</span>
      </div>

      <div style={{ textAlign: 'center', color: 'var(--text3)', fontSize: '12px' }}>{t('waiting_house', lang)}</div>
    </div>
  );
}
...
function PlayerSlot({ player, isMe, isReady, lang }) {
  ...
  return (
    <div style={{ background: isMe ? 'rgba(124,58,237,0.2)' : 'var(--bg3)', border: `2px solid ${isReady ? 'var(--success2)' : isMe ? 'var(--accent)' : 'var(--border)'}`, borderRadius: 'var(--radius-sm)', padding: '10px 6px', textAlign: 'center', animation: 'pop 0.3s ease forwards', position: 'relative' }}>
      ...
      <div style={{ width: '34px', height: '34px', borderRadius: '50%', background: isMe ? 'linear-gradient(135deg, var(--accent), var(--accent2))' : 'var(--surface2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 700, margin: '0 auto 5px', color: 'white' }}>{initials}</div>
...
function EmptySlot({ lang }) {
  return (
    <div style={{ background: 'var(--bg3)', border: '2px dashed var(--border)', borderRadius: 'var(--radius-sm)', padding: '10px 6px', textAlign: 'center', opacity: 0.4 }}>
