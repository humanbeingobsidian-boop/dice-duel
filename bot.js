...
function startBot() {
  ...
  bot.catch((err) => {
    const { error_code } = err.error || {};
    if (error_code === 409) {
      // Another instance is running — wait and retry
      console.warn('⚠️  Bot 409 conflict — another instance running, retrying in 5s...');
      setTimeout(() => {
        bot.start({
          onStart: (info) => console.log(`🤖 Bot @${info.username} running (retry)`),
  ...
  // הפעל polling עם retry אוטומטי על 409
  bot.start({
    onStart: (info) => console.log(`🤖 Bot @${info.username} running`),
  }).catch((err) => {
    if (err.error_code === 409) {
      console.warn('⚠️  Bot 409 on start — retrying in 10s...');
